...
% A note about fonts:
% The Cascadilla stylesheet mandates the use of Times or Times New Roman. In at
% least some TeX distributions (TeXLive being a known instance), compiling your
% document with `pdftex' produces a PDF that uses a font called `NimbusRom',
% which though extremely similar in appearance to Times, does not strictly
% conform to the stylesheet. I therefore recommend compiling your document with
% `latex', which produces a DVI file, and then converting that DVI file to a
% PDF, using either:
%   (i) `dvipdf', or 
%   (ii) `dvips' followed by one of (a) `ps2pdf' or (b) `pstopdf'.
% Either such method produces, in my experience, a PDF that references the
% desired `Times-Roman' Type 1 font.
...
% bottom of file). Note also the use of the \immedsubsection command.
